pipeline {
    ...
    parameters {
        ...
        string(name: 'IP', defaultValue: '10.32.50.22', description: 'default : 백오피스 서버')
    }
    stages {
        stage('1. Docker Run') {
            steps {
                withCredentials([
                    ...
                    sh """
                        ssh -i ${SSH_KEY} -p 50022 -o StrictHostKeyChecking=no jenkins@${params.IP} '
                            ...
                            echo "===== docker pull ====="
                            docker pull 10.32.50.26/dev/jira-reporter-frontend:${params.TAG}
                            docker pull 10.32.50.26/dev/jira-reporter-backend:${params.TAG}
                            docker pull 10.32.50.26/dev/mongo:latest
