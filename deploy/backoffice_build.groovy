...
echo "===== 4. 이미지 빌드 ====="
docker build -f ${env.REMOTE_DIR}/Dockerfile --build-arg BASE_IMAGE=${env.HARBOR_URL}/dev/python-base:3.12-slim --build-arg SKIP_SYS_DEPS=true --build-arg PIP_INDEX_URL=${env.PIP_INDEX_URL} -t ${env.HARBOR_URL}/dev/jira-reporter-backend:${params.TAG} ${env.REMOTE_DIR}/

docker build -f ${env.REMOTE_DIR}/frontend/optool/Dockerfile --build-arg NODE_BASE_IMAGE=${env.HARBOR_URL}/dev/node-base:22-alpine --build-arg NGINX_BASE_IMAGE=${env.HARBOR_URL}/dev/nginx:alpine --build-arg SKIP_NPM_INSTALL=true -t ${env.HARBOR_URL}/dev/jira-reporter-frontend:${params.TAG} ${env.REMOTE_DIR}/
