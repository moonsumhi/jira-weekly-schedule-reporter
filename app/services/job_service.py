...
async def _write_history(
    ...
    h = MongoClientManager.get_job_plans_history_collection()
    doc = {
        "plan_id": plan_id,
        "action": action,
...
class ServiceWorkPlanService:

    @staticmethod
    def _col():
        return MongoClientManager.get_job_plans_collection()
    ...
    async def patch(
        ...
        *,
        ...
        fields = [
            ...
            "purpose", "scope", "detail", "service_affected", "downtime",
            "impact_scope", "backup_done", "backup_details", "rollback_possible",
            "rollback_steps", "rollback_duration", "status", "result_notes",
        ]
        for field in fields:
        ...
        # nullable fields
        for nullable in ["downtime", "impact_scope", "backup_details",
                         "rollback_steps", "rollback_duration", "result_notes"]:
...
class NonServiceWorkPlanService:
    ...
    @staticmethod
    def _col():
        return MongoClientManager.get_job_plans_non_service_collection()

    @staticmethod
    def _hist():
        return MongoClientManager.get_job_plans_non_service_history_collection()
    ...
    async def create(self, *, data: Dict[str, Any], actor_email: str) -> Dict[str, Any]:
        col = self._col()
        now = TimeUtil.now_utc()
    ...
    async def get_history(self, *, plan_id: str) -> List[Dict[str, Any]]:
        cursor = self._hist().find({"plan_id": plan_id}).sort("changed_at", -1)
        items: List[Dict[str, Any]] = []
        async for doc in cursor:
            items.append(to_out(doc))
        return items
...
class JobResultService:
    ...
    @staticmethod
    def _col():
        return MongoClientManager.get_job_results_collection()

    @staticmethod
    def _hist():
        return MongoClientManager.get_job_results_history_collection()
    ...
    async def patch(
        ...
        *,
        ...
        fields = [
            "title", "work_date", "worker", "requester", "system_name", "category",
            "work_summary", "outcome", "status",
        ]
        for field in fields:
            if field in patch and patch[field] is not None:
                update[field] = patch[field]

        for nullable in ["issues_found", "resolution", "service_impact_actual",
                         "next_steps", "related_plan_id"]:
            if nullable in patch:
                update[nullable] = patch[nullable]
