# app/services/job_result_service.py
...
async def _write_history(
    ...
    h = MongoClientManager.get_job_results_history_collection()
    doc = {
        "result_id": result_id,
        "action": action,
...
class ServiceWorkResultService:
    ...
    async def create(
        ...
        *,
        ...
        if step_results and hasattr(step_results[0], "model_dump"):
            step_results = [s.model_dump() for s in step_results]

        doc = {
            **{k: v for k, v in data.items() if k != "step_results"},
    ...
    async def patch(
        ...
        *,
        ...
        fields = [
            ...
            "result", "actual_start_time", "actual_end_time", "summary",
            "service_affected", "actual_downtime",
            "issues_occurred", "issue_details", "action_taken",
            "post_check_done", "post_check_details", "plan_id", "notes",
        ]
        for field in fields:
        ...
        # nullable fields
        for nullable in [
            "actual_start_time", "actual_end_time", "actual_downtime",
            "issue_details", "action_taken", "post_check_details", "plan_id", "notes",
        ]:
            if nullable in patch:
                update[nullable] = patch[nullable]

        if "step_results" in patch and patch["step_results"] is not None:
            steps = patch["step_results"]
            if steps and hasattr(steps[0], "model_dump"):
                steps = [s.model_dump() for s in steps]
            update["step_results"] = steps
    ...
    async def get_history(self, *, result_id: str) -> List[Dict[str, Any]]:
        h = MongoClientManager.get_job_results_history_collection()
        cursor = h.find({"result_id": result_id}).sort("changed_at", -1)
        items: List[Dict[str, Any]] = []
