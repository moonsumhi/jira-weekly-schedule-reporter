"""
Migration script: update Job form templates to support multiple-row sections.
...
Changes applied:
  ...
  작업결과서:
    ...
    - "작업 대상" section     → multiple=true
    - "작업 내용" section     → multiple=true
    - "항목 추가" section     → multiple=true, add fields: 담당자, 테스트 결과, 테스트 실패 분석
...
def _update_result_sections(sections: list) -> list:
    """
    - Remove 작업 실적
    - Mark 작업 대상 / 작업 내용 / 항목 추가 as multiple=true
    - Ensure 항목 추가 has 담당자, 테스트 결과, 테스트 실패 분석 fields
    """
    updated = []
    for section in sections:
        if not isinstance(section, dict):
