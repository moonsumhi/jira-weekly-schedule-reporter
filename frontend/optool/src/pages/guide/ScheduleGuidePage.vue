        ...
        <div class="row items-start no-wrap q-gutter-md">
          ...
          <div class="col">
            ...
            <div v-if="si === 2 && idx === 1" class="mock-screen" style="max-width:480px">
              ...
              <div class="text-caption text-grey-5 q-mt-xs">Enter 또는 다른 곳 클릭(blur) 시 자동 저장 · 설명(textarea)도 동일하게 동작</div>
            </div>
          ...
          <q-card flat bordered>
            <q-card-section>
              ...
              <div class="column q-gutter-xs q-mb-sm" style="pointer-events:none">
                ...
                <div class="text-caption text-grey-5">이벤트 클릭 → 이슈 제목, 상태, 담당자, 마감일 팝업 확인</div>
...
const scenarios: Scenario[] = [
  {
    ...
    steps: [
      ...
      },
      {
        ...
        desc: '생성된 조직 카드를 클릭하면 조직 상세 페이지가 열립니다. 상단의 [프로젝트 추가] 버튼으로 새 프로젝트를 만듭니다. 프로젝트 이름(필수), 키(필수), 설명(선택)을 입력하고 저장합니다.',
      },
      {
  ...
  },
  {
    ...
    steps: [
      ...
      },
      {
        ...
        desc: '설정 탭에서 만들어둔 라벨을 선택해 이슈를 분류합니다. 설명란에 상세 작업 내용을 작성하고, 참고 자료는 드래그 앤 드롭 또는 버튼으로 첨부합니다.',
      },
      {
  ...
  },
  {
    ...
    steps: [
      ...
      {
        label: '제목·설명 인라인 편집',
        desc: '제목 또는 설명 영역을 클릭하면 바로 편집 모드로 전환됩니다. 제목은 Enter 키, 설명은 포커스 이동(blur) 시 자동 저장됩니다. 별도의 [저장] 버튼을 누를 필요가 없습니다.',
      },
      {
        label: '우측 사이드바에서 속성 수정',
        desc: '우측 사이드바(260px 고정)에서 상태, 타입, 우선순위, 담당자, 보고자, 스프린트, 상위 Epic, 스토리 포인트, 시작일, 마감일, 라벨을 수정합니다. 각 항목을 클릭하면 드롭다운 또는 날짜 선택기가 열립니다.',
      },
      {
      ...
      },
      {
        ...
        desc: '댓글 섹션에서 팀원과 소통합니다. 댓글에 [답글] 버튼으로 스레드형 답변을 달거나, [파일 첨부]로 자료를 올릴 수 있습니다. Ctrl+V로 클립보드 이미지를 바로 붙여넣는 기능도 지원합니다.',
      },
      {
  ...
  },
  {
    ...
    steps: [
      ...
      {
        label: '스프린트 필터 드롭다운으로 보기 범위 조절',
        desc: '보드 상단의 스프린트 선택 드롭다운에서 특정 스프린트 또는 [전체]를 선택합니다. 전체 선택 시 스프린트 배정 여부에 관계없이 모든 이슈가 표시됩니다.',
      },
      {
  ...
  },
  {
    ...
    steps: [
      {
        ...
        desc: '좌측 메뉴에서 스케줄 관리 → 주간 보고를 클릭합니다. 기존 보고서 목록(주차, 기간, 제목, 상태, 업무 현황 배지, 완료율 바)이 표시됩니다. 연도·주차 필터로 검색할 수 있으며, [목록 Excel]로 보고서 목록을 다운로드할 수 있습니다.',
      },
      {
        label: '[생성(자동 집계)] 클릭 → 연도·주차·제목·부서 입력',
        desc: '버튼을 누르면 다이얼로그가 열립니다. 연도와 주차를 입력하면 보고서 제목(예: 2026년 29주차 주간 보고)과 기간이 자동으로 생성됩니다. 부서명은 선택 항목입니다. [생성]을 누르면 해당 기간의 이슈가 자동으로 집계됩니다.',
      },
      {
      ...
      {
        label: '수기 항목 직접 추가',
        desc: '자동 집계 외에 주요 안건(카테고리·상태 포함), 특이사항 및 리스크(유형·영향도·대응 방안 포함), 결정 필요 사항(배경·선택지·희망 결정일 포함) 3개 섹션에 항목을 직접 추가할 수 있습니다.',
      },
      {
    ...
    tip: '보고서 제출 직전에 [재집계]를 한 번 눌러 최신 이슈 상태를 반영하세요. 집계 후 수기 항목과 관리자 코멘트를 추가하는 순서로 작성하면 효율적입니다.',
  },
]
