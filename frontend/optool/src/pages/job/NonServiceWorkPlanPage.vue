<template>
  <q-page class="q-pa-md">
    ...
    <div class="row items-center q-gutter-sm q-mb-md">
      <div class="text-h6">작업계획서 (서비스 외)</div>
      <q-space />
      ...
      <q-card style="width: 800px; max-width: 95vw; max-height: 90vh; display: flex; flex-direction: column">
        ...
        <q-card-section class="col scroll">
          ...
          <div class="row q-gutter-sm q-mt-xs">
            ...
            <q-input v-model="form.system_name" outlined dense label="시스템명/위치 *" class="col-12" />
          </div>
    ...
    <q-dialog v-model="detailDialog">
      <q-card v-if="detailRow" style="width: 700px; max-width: 95vw; max-height: 90vh" class="column">
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h6">작업계획서 상세 (서비스 외)</div>
          <q-space />
          <q-btn flat dense icon="close" v-close-popup />
        </q-card-section>
        ...
        <q-card-section class="col scroll">
          ...
          <div class="row q-col-gutter-sm q-mb-sm">
            ...
            <div class="col-12">
              <div class="text-caption text-grey-7">시스템명/위치</div>
...
import {
  listNonServiceWorkPlans,
  createNonServiceWorkPlan,
  patchNonServiceWorkPlan,
  deleteNonServiceWorkPlan,
...
const rows = ref<NonServiceWorkPlan[]>([])
...
const columns: NonNullable<QTableProps['columns']> = [
  ...
  { name: 'system_name', label: '시스템명/위치', field: 'system_name', align: 'left', sortable: true },
...
async function load() {
  loading.value = true
  try {
    rows.value = await listNonServiceWorkPlans(includeDeleted.value)
  } catch (err: unknown) {
...
const selectedRow = ref<NonServiceWorkPlan | null>(null)
...
function openEdit(row: NonServiceWorkPlan) {
  isEdit.value = true
...
async function doCreate() {
  ...
  try {
    const payload: NonServiceWorkPlanCreate = {
...
async function doEdit() {
  ...
  try {
    const updated = await patchNonServiceWorkPlan(selectedRow.value.id, {
      title: form.title.trim(),
      work_date: form.work_date.trim(),
      worker: form.worker.trim(),
      requester: form.requester.trim(),
...
/** Delete */
function confirmDelete(row: NonServiceWorkPlan) {
  $q.dialog({
    title: '삭제',
    message: `정말 삭제하시겠습니까?\n${row.title}`,
    cancel: true,
...
async function doDelete(row: NonServiceWorkPlan) {
  actingId.value = String(row.id)
  actingType.value = 'delete'
  try {
    await deleteNonServiceWorkPlan(row.id)
    rows.value = rows.value.map((r) =>
...
const detailRow = ref<NonServiceWorkPlan | null>(null)

function openDetail(row: NonServiceWorkPlan) {
  detailRow.value = row
