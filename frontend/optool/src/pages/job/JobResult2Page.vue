  ...
  <q-page class="q-pa-md">
    ...
    <q-dialog v-model="formDialog" persistent>
      <q-card style="width: 800px; max-width: 95vw; max-height: 90vh" class="column">
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h6">{{ isEdit ? '작업결과서2 수정' : '작업결과서2 추가' }}</div>
          <q-space />
    ...
    <q-dialog v-model="detailDialog">
      <q-card v-if="detailRow" style="width: 700px; max-width: 95vw; max-height: 90vh" class="column">
        <q-card-section class="row items-center q-pb-none">
          <div class="text-h6">작업결과서2 상세</div>
          <q-space />
          <q-btn flat dense icon="close" v-close-popup />
        </q-card-section>
...
async function doCreate() {
  ...
  try {
    ...
    $q.notify({ type: 'positive', message: '작업결과서2가 생성되었습니다.' })
  } catch (err: unknown) {
