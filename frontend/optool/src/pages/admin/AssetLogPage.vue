<template>
  <q-page padding>
    <div class="text-h6 q-mb-md">자산 로그</div>
      ...
      <q-card-section>
        <div class="row q-col-gutter-sm items-end">
          ...
          <div class="col-6 col-sm-2">
            <q-select v-model="filterCategory" dense outlined label="자산 유형" :options="categoryOptions" emit-value map-options />
          </div>
          <div class="col-6 col-sm-2">
        ...
        <q-markup-table flat dense separator="horizontal">
          ...
          <tbody>
            ...
            <template v-else-if="items.length === 0">
              <tr>
                <td colspan="6" class="text-center text-grey-6 q-pa-lg">데이터가 없습니다.</td>
              </tr>
            </template>
            <template v-else>
        ...
        <q-card style="min-width: 480px; max-width: 640px">
          ...
          <q-card-section v-if="detailItem">
            <q-list dense>
              ...
              <q-item>
                <q-item-section>
                  <q-item-label caption>자산 유형</q-item-label>
                  <q-item-label>
            ...
            <div v-if="detailItem.diff && detailItem.diff.length">
              <!-- CREATE: 항목/추가된 값 2열 -->
              <q-markup-table v-if="detailItem.action === 'CREATE'" flat dense bordered>
                <thead>
...
function buildParams(pageNum: number, pageSize: number): Record<string, string | number> {
  const params: Record<string, string | number> = { page: pageNum, page_size: pageSize }
  ...
  // 특정 자산 유형이 선택된 경우 category, 아니면 category_group=assets
  if (filterCategory.value) {
    params.category = filterCategory.value
  } else {
    params.category_group = 'assets'
  }
  return params
}
...
async function load() {
  loading.value = true
  try {
    const res = await api.get<{ total: number; items: AuditLogItem[] }>('/admin/audit-log', { params: buildParams(page.value, PAGE_SIZE) })
...
async function doExport() {
  ...
  try {
    const res = await api.get<{ total: number; items: AuditLogItem[] }>('/admin/audit-log', { params: buildParams(1, 10000) })
    ...
    ws['!cols'] = [{ wch: 22 }, { wch: 28 }, { wch: 16 }, { wch: 14 }, { wch: 10 }, { wch: 60 }]
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, ws, 'AssetLog')
    ...
    a.href = url
    a.download = `asset_log_${dateStr}.xlsx`
    a.click()
