...
const HELP: Record<string, HelpContent> = {
  ...
  'work-management': {
    title: '작업 관리',
    summary: '작업계획서와 작업결과서를 등록하고, 원본 문서를 Import한 뒤 누락된 항목을 확인하는 화면입니다.',
    steps: [
      { label: '문서 유형 선택', desc: '상단의 전체·작업계획서·작업결과서 탭에서 작성하거나 확인할 문서 유형을 선택합니다.', color: 'primary' },
      { label: '새 문서 추가', desc: '[파일 추가]를 눌러 템플릿을 선택하고 입력합니다. 한 필드 안에서 글과 이미지를 함께 작성할 수 있습니다.', color: 'primary' },
      { label: '원본 파일 Import', desc: '[Import]를 누른 후 HWP·HWPX·Word 파일을 선택합니다. 전체 화면에서는 먼저 적용할 템플릿을 선택합니다.', color: 'teal' },
      { label: '상세 → 수정', desc: '목록에서 [상세]를 열고 [수정]을 누르면 문서 양식 그대로 내용을 고칠 수 있습니다. 이미지 붙여넣기와 Ctrl+V도 지원합니다.', color: 'orange' },
      { label: 'Import 결과 확인', desc: '매핑되지 않은 내용이 있으면 [에러 메시지]에서 비어 있는 템플릿 위치를 확인하고, [가져온 추가 내용]에서 원본 내용을 확인합니다.', color: 'negative' },
      { label: '저장 및 내려받기', desc: '입력을 마친 뒤 [저장]합니다. 상세 화면에서 Markdown 보기, 원본 파일 다운로드, HWP 내보내기를 사용할 수 있습니다.', color: 'positive' },
    ],
    tip: 'Import 후에는 작업 결과 > 작업 전·작업 후처럼 빈 항목이 없는지 먼저 확인하면 누락을 줄일 수 있습니다.',
  },
}
</script>
